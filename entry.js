...
INVALID_CAPTAIN_CODE:'대표 코드가 유효하지 않거나 본인 계정에 발급된 코드가 아닙니다.',INVALID_PARTNER_CODE:'팀원 코드가 유효하지 않습니다. 대표 코드와 다른 회원의 코드를 입력해 주세요.',CODE_ALREADY_USED:'이미 신청에 사용된 코드입니다.',
...
async function refreshEntry(){
 ...
 entryEl('myCodePanel').hidden=!s.code;entryEl('myCode').textContent=s.code||'';
 entryEl('invitePanel').hidden=!s.invites?.length;
 entryEl('inviteCards').innerHTML=(s.invites||[]).map(i=>'<article class="entry-panel"><h2>'+esc(i.team_name)+'</h2><p>'+esc(divisionLabel(i.division))+' / '+esc(categoryName(i.category))+'<br>대표: '+esc(i.captain)+'</p><p>팀 참가비는 입금 확인되었습니다. 추가 입금 없이 초대를 수락해 합류하세요.</p>'+(!playerProfile?.profile_complete?'<p><a href="player-profile.html?next=athlete.html">먼저 선수 정보 작성 →</a></p>':'')+'<label class="entry-check"><input type="checkbox" id="consent-'+esc(i.id)+'"><span><a href="terms.html" target="_blank" rel="noopener">이용약관</a>·<a href="privacy.html" target="_blank" rel="noopener">개인정보처리방침</a>·<a href="refund.html" target="_blank" rel="noopener">환불·취소정책</a>을 확인하고 팀 참가에 동의합니다.</span></label><div class="entry-actions"><button class="btn arcade-primary" data-respond="'+esc(i.id)+'" data-accept="true">초대 수락</button><button class="btn dark" data-respond="'+esc(i.id)+'" data-accept="false">거절</button></div></article>').join('');
...
function renderEntryStatus(t){
 ...
 else if(t.is_captain)content+='<form id="paymentForm"><label>실제 입금자명<input id="depositorName" required minlength="2" maxlength="50" autocomplete="name"></label><label class="entry-check"><input type="checkbox" required><span>위 팀 합계 금액을 입금했습니다.</span></label><button class="btn arcade-primary" type="submit">입금 완료 알리기</button></form>';
...
entryEl('captainCode').addEventListener('input',invalidateQuote);entryEl('partnerCode').addEventListener('input',invalidateQuote);
entryEl('quoteButton').onclick=()=>runEntryAction(async()=>{entryQuote=await entryRpc('quote',{captain_code:entryEl('captainCode').value.trim(),partner_code:entryEl('partnerCode').value.trim()});entryEl('priceQuote').innerHTML=esc(entryQuote.pricing_tier==='early_bird'?'EARLY BIRD':'REGULAR')+' · 대표 '+won(entryQuote.captain_fee)+' + 팀원 '+won(entryQuote.partner_fee)+'<br>입금할 팀 합계 <strong>'+won(entryQuote.amount_due)+'</strong>'+(entryQuote.partner_name?'<p>팀원 할인: '+esc(entryQuote.partner_name)+'</p>':'');});
entryEl('entryForm').onsubmit=e=>{e.preventDefault();if(!entryQuote||!e.target.reportValidity())return;runEntryAction(async()=>{
 ...
 await entryRpc('register',{team_name:entryEl('entryTeam').value.trim(),division:entryEl('entryDivision').value,category:entryEl('entryCategory').value,captain_code:entryEl('captainCode').value.trim(),partner_code:entryEl('partnerCode').value.trim(),expected_amount:entryQuote.amount_due,consent:entryEl('entryConsent').checked});
