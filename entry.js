...
function renderEntryStatus(t){
 ...
 if(paid){content+='<div class="arc-roster"><div><span>CAPTAIN</span><strong>'+esc(t.player_1||'CAPTAIN')+'</strong></div><div><span>PARTNER</span><strong>'+esc(t.roster_complete?t.player_2:t.pending_invite?.name||'INVITE PARTNER')+'</strong></div></div>';if(t.roster_complete){content+='<p>Both athletes are on the team.</p>';}else if(t.is_captain){content+='<p>Invite your partner using their ARC account email. You can then confirm their place without a separate acceptance.</p>';if(t.pending_invite)content+='<p>AWAITING CONFIRMATION: '+esc(t.pending_invite.name)+'</p><div class="entry-actions"><button class="btn arcade-primary" data-entry-action="captain_confirm">CONFIRM PARTNER</button><button class="btn dark" data-entry-action="cancel_invite">CANCEL INVITE</button></div>';else content+='<form id="inviteForm"><label>PARTNER ARC ACCOUNT EMAIL<input id="inviteEmail" type="email" autocomplete="off" required></label><button class="btn arcade-primary" type="submit">INVITE PARTNER</button></form>';}}
