...
PROFILE_REQUIRED:'Complete your player profile first.',PARTNER_PROFILE_REQUIRED:'Your partner needs a player name, gender and phone. Ask them to complete their profile, or contact staff.',CONSENT_REQUIRED:'Required consent is missing.',TEAM_NAME_REQUIRED:'Enter a team name.',INVALID_DIVISION:'Choose a division.',INVALID_CATEGORY:'Choose a category.',
...
PAYMENT_NOT_CONFIRMED:'Add your partner after staff confirms the bank transfer.',CAPTAIN_ONLY:'Only the captain can do this.',DISCOUNT_PARTNER_ONLY:'The selected partner is not eligible for this entry. Contact us for help.',
...
function renderEntryStatus(t){
 ...
 if(paid){content+='<div class="arc-roster"><div><span>CAPTAIN</span><strong>'+esc(t.player_1||'CAPTAIN')+'</strong></div><div><span>PARTNER</span><strong>'+esc(t.roster_complete?t.player_2:t.pending_invite?.name||'ADD PARTNER')+'</strong></div></div>';if(t.roster_complete){content+='<p>Both athletes are on the team.</p>';}else if(t.is_captain){content+='<p>Enter your partner’s ARC account email to add them immediately. No separate acceptance is needed. Make sure you have agreed to participate together.</p>';if(t.pending_invite)content+='<p>READY TO ADD: '+esc(t.pending_invite.name)+'</p><div class="entry-actions"><button class="btn arcade-primary" data-entry-action="captain_confirm">ADD PARTNER NOW</button><button class="btn dark" data-entry-action="cancel_invite">CANCEL INVITE</button></div>';else content+='<form id="inviteForm"><label>PARTNER ARC ACCOUNT EMAIL<input id="inviteEmail" type="email" autocomplete="off" required></label><button class="btn arcade-primary" type="submit">ADD PARTNER</button></form>';}}
...
document.addEventListener('submit',ev=>{
 if(ev.target.id==='inviteForm'){ev.preventDefault();runEntryAction(async()=>{await entryRpc('add_partner',{team_id:entryState.entry.id,email:entryEl('inviteEmail').value.trim()});await refreshEntry();entryMessage('Partner added. Your team is complete; no separate acceptance is needed.');});}
...
document.addEventListener('click',ev=>{
 ...
 if(b.dataset.entryAction){const action=b.dataset.entryAction;if(action==='cancel'&&!confirm('Cancel this unpaid entry? If you have transferred, contact us instead.'))return;if(action==='captain_confirm'&&!confirm('Confirm '+entryState.entry.pending_invite.name+' as your partner? Only confirm if you have agreed to participate together. They will not need to accept separately.'))return;const payload={team_id:entryState.entry.id,invite_id:entryState.entry.pending_invite?.id};runEntryAction(async()=>{if(action==='captain_confirm'){const r=await db.rpc('arc_confirm_partner',{p_invite_id:payload.invite_id});if(r.error)throw r.error;}else await entryRpc(action,payload);await refreshEntry();entryMessage(action==='captain_confirm'?'Partner confirmed. Your team is complete.':action==='cancel'?'Entry cancelled.':'Invitation cancelled.');});}
